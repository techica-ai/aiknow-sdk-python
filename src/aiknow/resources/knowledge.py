...
class KnowledgeResource:
    """Synchronous knowledge search resource (deprecated).

    Access via ``client.knowledge.search()``.

    Args:
        client:    Shared ``httpx.Client`` (lifecycle owned by AIKnowClient).
        tenant_id: Default tenant. Can be overridden per-call.
    """
    ...
    def search(
        ...
        """Search the Knowledge Base for documents relevant to *query* (sync).
        ...
        try:
            # Strategy 1: dedicated retrieval endpoint
            if self._has_retrieval_endpoint is not False:
                chunks = self._search_via_retrieval(
                    query=query,
                    top_k=top_k,
                    tenant_id=effective_tenant,
                    filters=filters,
    ...
    def _search_via_retrieval(
        ...
        """Call POST /retrieval/search (sync). Returns None if endpoint unavailable."""
        payload: dict[str, Any] = {"query": query, "top_k": top_k}
        if tenant_id:
            payload["tenant_id"] = tenant_id
        if filters:
        ...
        try:
            response = self._client.post("/retrieval/search", json=payload, timeout=5.0)
        except Exception as exc:
            logger.debug("Knowledge[retrieval]: request failed (%s) — switching to fallback", exc)
            self._has_retrieval_endpoint = False
            return None
    ...
    def _search_via_chat(
        ...
        """Fallback: POST /chat/ask and extract citations from DoneEvent (sync)."""
        payload: dict[str, Any] = {"query": query}
        if tenant_id:
            payload["tenant_id"] = tenant_id

        try:
            response = self._client.post("/chat/ask", json=payload, timeout=8.0)
        except Exception as exc:
            logger.warning("Knowledge[chat-fallback]: request failed: %s", exc)
            return []
